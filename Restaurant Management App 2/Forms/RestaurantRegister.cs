...
namespace Restaurant_Management_App
{
    public partial class RestaurantRegister : Form
    {
        ...
        private int CheckDataAndRegister()
        {
            ...
            con.Open();
            if (new MySqlCommand($"SELECT username FROM Managers WHERE username='{txt_mname.Text}'", con.GetCon()).ExecuteScalar() == null)
            {
                con.Close();
                throw new Exception("Manager does not exist!");
            }

            if (new MySqlCommand($"SELECT id FROM Managers WHERE username='{txt_mname.Text}' AND password='{txt_mpassword.Text}'", con.GetCon()).ExecuteScalar() == null)
            {
                con.Close();
                throw new Exception("Incorrect password!");
            }
            int id_manager = (int)new MySqlCommand($"SELECT id FROM Managers WHERE username='{txt_mname.Text}' AND password='{txt_mpassword.Text}'", con.GetCon()).ExecuteScalar();

            new MySqlCommand($"INSERT INTO Restaurants(name,password,L,W,tables,id_manager) " +
                $"VALUES('{txt_rname.Text}','{txt_rpassword.Text}',{Convert.ToDouble(txt_L.Text)},{Convert.ToDouble(txt_W.Text)},'',{id_manager})",con.GetCon()).ExecuteNonQuery();


            int id = (int)new MySqlCommand($"SELECT id FROM Restaurants WHERE name='{txt_rname.Text}' AND password='{txt_rpassword.Text}'", con.GetCon()).ExecuteScalar();
            con.Close();
            return id;
        }
    }
